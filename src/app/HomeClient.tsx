...
export default function HomeClient({ initialPosts, content }: HomeClientProps) {
  ...
  // Abrir FAQ automáticamente después de 30 segundos
  useEffect(() => {
    const timer = setTimeout(() => setIsFaqOpen(true), 30000);
  ...
  const faqs = [
    {
      q: c('faq_1_q', "¿Cómo son las sesiones 1 a 1?"),
      a: c('faq_1_a', "Son encuentros confidenciales (virtuales o presenciales) de 60 minutos. Exploramos en profundidad aquello que querés desarmar o mirar distinto, al ritmo que necesites y sin fórmulas rígidas."),
    },
    {
      q: c('faq_2_q', "¿En qué se diferencia el Programa 'Aprender a Aprender'?"),
      a: c('faq_2_a', "No es un curso teórico. Es un recorrido inmersivo de 4 meses donde integramos el cuerpo, la emoción y el lenguaje a través de talleres grupales, rutas individuales y acompañamiento personal."),
    },
    {
      q: c('faq_3_q', "¿Necesito conocimientos previos?"),
      a: c('faq_3_a', "En absoluto. Solo disposición para cuestionar el automático y apertura para transitar un proceso de aprendizaje."),
    },
    {
      q: c('faq_4_q', "¿Qué medios de pago aceptan?"),
      a: c('faq_4_a', "Aceptamos transferencias bancarias locales e internacionales. Para el programa podés consultar nuestros planes de financiación en cuotas."),
    },
  ];
  ...
  const handleContactSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const correo = (form.querySelector('#correo') as HTMLInputElement).value.trim();
    const asunto = (form.querySelector('#asunto') as HTMLInputElement).value.trim();
    const mensaje = (form.querySelector('#mensaje') as HTMLTextAreaElement).value.trim();

    // Guardar en CRM (fire-and-forget) — incluye honeypot para que la API lo valide
    const website = (form.querySelector('input[name="website"]') as HTMLInputElement)?.value ?? '';
    fetch('/api/contact', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: nombre, email: correo, subject: asunto, message: mensaje, website }),
    }).catch(() => {});

    const text = `Hola Erika! Te escribo desde tu web.\n\nNombre: ${nombre}\nEmail: ${correo}\nAsunto: ${asunto}\n\nMensaje: ${mensaje}`;
    window.open(`https://wa.me/5493572440360?text=${encodeURIComponent(text)}`, '_blank');
    setContactSent(true);
    form.reset();
  };
      ...
      <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 overflow-hidden flex items-center min-h-[90vh]">
        <div className="max-w-7xl mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tighter leading-[1.05] mb-6 text-brand-olive-900">
      ...
      <section id="contacto" className="py-24 bg-stone-900 text-stone-50">
        ...
        >
          ...
          <div className="bg-stone-800 p-8 rounded-2xl border border-stone-700 shadow-xl">
            <h3 className="text-xl font-bold mb-6 text-white block border-b border-stone-700 pb-4">Dejame tu mensaje</h3>
            {contactSent && (
              <div className="mb-4 p-4 bg-brand-olive/20 border border-brand-olive rounded-lg text-brand-olive-light text-sm font-medium">
                ¡Gracias! Se abrió WhatsApp con tu mensaje. Te respondo pronto.
              </div>
            )}
            <form className="space-y-4" onSubmit={handleContactSubmit}>
              {/* Honeypot — invisible para humanos, los bots lo llenan y se ignora */}
              <input type="text" name="website" tabIndex={-1} autoComplete="off" style={{ display: 'none' }} />
              <div>
                <label className="block text-sm font-medium text-stone-400 mb-1" htmlFor="nombre">Nombre completo</label>
                <input type="text" id="nombre" required className="w-full bg-stone-900 border border-stone-700 rounded-lg px-4 py-3 text-stone-100 placeholder:text-stone-600 focus:outline-none focus:border-brand-taupe focus:ring-1 focus:ring-amber-500 transition-all" placeholder="Juan Pérez" />
              </div>
              <div>
